...
module.exports = (client) => {
  ...
  const getRole = (guild) => {
    return guild.roles.cache.find((role) => role.name === 'Zilītis (sēž kaktā)')
  }
    ...
    try {
      redisClient.get(`${redisKeyPrefix}${id}-${guild.id}`, (err, result) => {
        if (err) {
          ...
          console.log('civēks netika kaktā iebāzts')
        }
      })
    } finally {
      redisClient.quit()
  ...
  command(client, 'kakts', async (message) => {
  ...
  const role2 = guild.roles.cache.get("862419538331303937")
    ...
    if (!target) {
      channel.send('Tu pats sevi ielikt kaktā gribi?')
      return
    }
    ...
    channel.send(`kaktā ielikts ${targetMember}`)
