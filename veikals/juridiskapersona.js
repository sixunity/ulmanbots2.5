...
module.exports = (client) => {
  const redisKeyPrefix = 'juridiska-'
  ...
  const getRole = (guild) => {
    return guild.roles.cache.find((role) => role.name === 'Juridiska persona')
  }
  ...
  command(client, 'juridiskapersona', async (message) => {
  ...
  const role2 = guild.roles.cache.get("862419538331303937")
    ...
    channel.send(`${targetMember} Ieguva Nodokļu atbrīvojumu`)
